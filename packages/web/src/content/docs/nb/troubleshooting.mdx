...
Hvis du har `OPENCODE_SERVER_PORT` satt i miljøet ditt, vil skrivebordsappen prøve å bruke den porten for den lokale serveren.

- Deaktiver `OPENCODE_SERVER_PORT` (eller velg en ledig port) og start på nytt.
