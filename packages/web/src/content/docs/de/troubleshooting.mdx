...
Wenn du `OPENCODE_SERVER_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENCODE_SERVER_PORT` zurueck (oder waehle einen freien Port) und starte neu.
