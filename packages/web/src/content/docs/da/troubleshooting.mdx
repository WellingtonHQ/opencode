...
Hvis du har `OPENCODE_SERVER_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `OPENCODE_SERVER_PORT` (eller vælg en ledig port) og start på ny.
